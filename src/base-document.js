...
const { isSchema, assert } = require('@hapi/joi');
 ...
 * @description the normalizeType function accepts an object or string property and
 * returns a object with a type property.
 * @param  {Object|String} property [description]
...
/**
 * @class BaseDocument
 * @description The BaseDocument class sets the
 */
class BaseDocument {
  /**
   * @constructor
   * @description the constuctor creates an id property and a _schema property
   * when a new base document is extended.
   */
  constructor() {
    this._schema = {
  ...
  static _fromData(datas) {
    ...
    datas.forEach(function(d) {
      const instance = that._instantiate();
  ...
  getDefault(schemaProp) {
    if (
      schemaProp in this._schema &&
      'default' in this._schema[schemaProp] &&
      !isSchema(this._schema[schemaProp])
    ) {
