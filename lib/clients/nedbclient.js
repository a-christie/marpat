...
class NeDbClient extends DatabaseClient {
    ...
    static canHandle(url) {
        const match = (url.indexOf('nedb://') === 0);
        ...
        try {
          ...
          throw new Error('nedb module is not installed.');
        }
    ...
    deleteMany(collection, query) {
        const that = this;
        return new Promise(function(resolve, reject) {
            const db = getCollection(collection, that._collections, that._path, that._options);
        ...
        return new Promise(function(resolve, reject) {
            ...
            that.findOne(collection, query).then(function(data) {
                if (!data) {
                    ...
                    return db.update(query, { $set: values }, function(
                        error,
                        result
                    ) {
                        if (error) return reject(error);
