...
const { connect, Document, EmbeddedDocument } = require('../index');
    ...
    it('should not load duplicate references in array when only one reference is present', function(done) {
      ...
      eye1
        ...
        .then(function(users) {
          ...
          let e1 =
            String(u1.eyes[0]._id) === String(eye1._id)
              ? u1.eyes[0]
              : u1.eyes[1];
          let e2 =
            String(u1.eyes[1]._id) === String(eye2._id)
              ? u1.eyes[1]
              : u1.eyes[0];
  ...
  describe('#43', function() {
    ...
    it('should save changes made in postValidate hook', function(done) {
      ...
      person
        ...
        .then(function() {
          validateId(person);
          return Person.findOne({ _id: person._id }, { populate: true }).then(
            p => {
      ...
      person
        ...
        .then(function(person) {
          return Person.findOneAndUpdate(
            { _id: person._id },
            { name: 'John Derp', 'contact.phone': '0123456789' }
          );
        })
        .then(function(person) {
